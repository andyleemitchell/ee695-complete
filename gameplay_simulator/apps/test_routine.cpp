...
std::vector<std::pair<long, std::string>> test_config = {
    {20, "20 iterations"},
    {20, "20 iterations"},
    {20, "20 iterations"},
    {20, "20 iterations"},
...
void showContinueScreen(raylib::Window& window, const std::string& message) {
    raylib::Text messageText(message, 20, RAYWHITE);

    Vector2 textSize = messageText.MeasureEx();
    
    while (!window.ShouldClose() && !IsKeyPressed(KEY_ENTER)) {
        BeginDrawing();
        ClearBackground(BLACK);
        
        messageText.Draw(
            (window.GetWidth() - textSize.x) / 2.0f,
            (window.GetHeight() - textSize.y) / 2.0f
        );
...
bool saveStringsToUniqueFile(const std::vector<std::string>& data, const std::string& directory) {
    auto now = std::chrono::system_clock::now();
    auto in_time_t = std::chrono::system_clock::to_time_t(now);

    std::stringstream ss;
    // Format: YYYYMMDD_HHMMSS. A simple format suitable for filenames.
    // Note: std::put_time requires C++11.
    ss << std::put_time(std::localtime(&in_time_t), "%Y%m%d_%H%M%S");

    std::string filename = ss.str() + ".txt";
    std::string full_path = directory;

    // Append directory separator if needed (basic check)
    ...
    if (!output_file.is_open()) {
        // File could not be opened (e.g., directory doesn't exist, permissions)
        return false;
    }
    ...
    // File is automatically closed when output_file goes out of scope
...
int main(void) {
    ...
    const char* winner1 = "Player 1 wins. Press <R>";
    const char* winner2 = "Player 2 wins. Press <R>";
    const char* tie_game = "Tied Game. Press <R>";
    ...
    int order_counter = 0;
    bool written_result = false;

    for (auto &config : test_config) {
        written_result = false;
        ...
        if (order_counter % 2 == 0) {
            p1 = std::make_unique<RaylibHuman>(eventQueue);
            p2 = std::make_unique<MonteCarloTreeSearch>(config.first, 1);
            orders.push_back("second");
        } else {
            p1 = std::make_unique<MonteCarloTreeSearch>(config.first, 2);
            p2 = std::make_unique<RaylibHuman>(eventQueue);
            orders.push_back("first");
        }
        c4sim::Game game(std::move(p1), std::move(p2));
        ++order_counter;

        showContinueScreen(window, "Press ENTER to start the Connect Four game. (" + config.second + ")");
        ...
        if (status == GameStatus::PLAYER1_WIN) {
            if (not written_result) {
                if (order_counter % 2 == 0) {
                    results.push_back("WIN");
                } else {
                    results.push_back("LOSE");
                }
                written_result = true;
            }
        }
    ...
    saveStringsToUniqueFile(results, "/home/andy/work/c4sim/test_results");
