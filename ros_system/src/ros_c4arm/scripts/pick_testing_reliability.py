    ...
    def __init__(self, num_picks=5):
        ...
        while True:
            ...
            if colour in ["R", "r", "Y", "y", "red", "Red", "yellow", "Yellow"]:
                ...
                run_num = int(input("How many picks to perform:"))
                if 0 < run_num < 100:
        ...
        while not rospy.is_shutdown():
            ...
            while True:
                success = input("Enter the result of pick: [S]uccess/[F]ailure/[P] partial failure: ")
                if success in ["s", "S"]:
                    success = "successful"
                    reason = ""
                    break
                if success in ["p", "P", "f", "F"]:
                    ...
                    print("[1]: couldn't plan to counter location (outside planning area)")
                    print("[2]: went to incorrect location")
                    print("[3]: failed to grasp counter")
                    print("[4]: counter misorientated in gripper")
                    temp = int(input("reason"))
                    if 1 <= temp <= 4:
                        if temp == 1:
                            reason = "counter outside planning area"
                        elif temp == 2:
                            reason = "incorrect location"
                        elif temp == 3:
                            reason = "failed to grasp counter"
                        elif temp == 4:
                            reason = "counter misorientated in gripper"
                    break
            ...
            if self.counter >= self.num_picks:
                ...
                with open(self.results_dir+"/pick_test_results_"+str(self.num_picks)+"_"+time.strftime("%Y%m%d-%H%M%S")+".json", "w") as f:
