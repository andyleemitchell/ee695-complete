...
class CounterPicker():
    def __init__(self, num_picks=5):
        ...
        self.results_dir = "/home/andy/work/ros_work/src/ros_c4arm/test_results"
        ...
        # get colour of counters and number of picks to do
        while True:
            colour = input("Enter the colour counters we are working with. [R/Y]:")
            if colour in ["R", "r", "Y", "y", "red", "Red", "yellow", "Yellow"]:
                self.stats.append({
                    "counter_colours": "red" if colour in ["R", "r", "red", "Red"] else "yellow"
                })
                self.colour_searching = "red" if colour in ["R", "r", "red", "Red"] else "yellow"
                run_num = int(input("How many places to perform:"))
                if 0 < run_num < 22:
                    self.num_picks = run_num
                    break
        ...
        while not rospy.is_shutdown():
            ...
            while True:
                success = input("Enter the result of place: [S]uccess/[F]ailure/[P] partial failure: ")
                if success in ["s", "S"]:
                    success = "successful"
                    reason = ""
                    break
                if success in ["p", "P", "f", "F"]:
                    success = "failure" if success in ["f", "F"] else "partial failure"
                    print("Enter reason for (partial) failure:")
                    print("[1]: didn't pick up counter")
                    print("[2]: counter dispensed out of range")
                    print("[3]: counter not dispensed (dispenser stuck)")
                    print("[4]: counter fell short of board")
                    print("[5]: counter fell long of board")
                    print("[6]: counter bounced out of slot")
                    print("[7]: went to wrong slot")
                    temp = int(input("reason: "))
                    if 1 <= temp <= 7:
                        if temp == 1:
                            reason = "didn't pick up counter"
                        elif temp == 2:
                            reason = "counter dispensed out of range"
                        elif temp == 3:
                            reason = "counter not dispensed (dispenser stuck)"
                        elif temp == 4:
                            reason = "counter fell short of board"
                        elif temp == 5:
                            reason = "counter fell long of board"
                        elif temp == 6:
                            reason = "counter bounced out of slot"
                        elif temp == 7:
                            reason = "went to wrong slot"
                    break
            ...
            self.column = (self.column + 1) % 7
            self.counter += 1
            if self.counter >= self.num_picks:
                rospy.loginfo("Finished test. Saving results.")
                with open(self.results_dir+"/full_test_results_"+str(self.num_picks)+"_"+time.strftime("%Y%m%d-%H%M%S")+".json", "w") as f:
                    json.dump(self.stats, f)
                exit()

            if self.counter % 7 == 0:
                input("Make sure there is at least 7 counters in dispenser: ")
            # else:
            #     input("Place new counter. Then press enter to do next pick: ")
