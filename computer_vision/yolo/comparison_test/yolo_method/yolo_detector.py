...
MODEL = "/home/andy/work/cv-nn-testing/weights/april16.pt"

class ConnectFourYoloDetector:
    def __init__(self, conf_thresh=0.8):
        self.model = YOLO(MODEL)
        self.class_names = self.model.names
        self.target_classes = ['Red Piece', 'Yellow Piece', 'No Piece']
        self.target_class_ids = [
            class_id for class_id, class_name in self.class_names.items() if class_name in self.target_classes
        ]
    ...
    def _process_detections(self):
        ...
        detections = [((detection[0]+detection[2])//2, (detection[1]+detection[3])//2, detection[4], detection[5]) for detection in self.detections[0].boxes.data]
        filtered_detections = [d for d in detections if d[2] >= self.confidence_threshold]
        ...
        if len(state) == 42:
            ...
            for row in range(6):
                row_entries = []
                for col in range(7):
                    row_entries.append(state[row*7 + col])
    ...
    def _show_debug_image(self, frame, num_detections):
        for result in self.detections[0].boxes.data:  # Each detection (x, y, width, height, confidence, class_id)
            x1, y1, x2, y2, confidence, class_id = result
            
            if int(class_id) in self.target_class_ids and float(confidence) > self.confidence_threshold:
                x1 = int(x1)
                y1 = int(y1)
                x2 = int(x2)
                y2 = int(y2)
        ...
        (text_width, text_height), baseline = cv2.getTextSize(str(num_detections), cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
        cv2.rectangle(frame, (10, 30 - 20), (10 + text_width, 10), (0, 0, 0), -1)
        cv2.putText(frame, str(num_detections), (10, 30 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    ...
    def detect(self, frame):
        ...
        self.detections = self.model(frame, verbose=False)
        num_detections = self._process_detections()
        # for i in range(len(self.board_state)):
        #     for j in range(len(self.board_state[i])):
        #         print(f"{self.board_state[i][j]}", end="\t")
        #     print()
        # self._show_debug_image(frame, num_detections)
        # return 6*[7*["empty"]]
